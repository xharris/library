...
-- -- inserted values must be comparable using < and <= (optionally ==)
...
-- gt == not lt and not le

local function insert(self, v)
	local history = {}
	local cur = self._head
	local gt = function(a, b) return self.lt(b, a) end
	while cur do
		if not cur.next or gt(cur.next.value, v) then 
			-- move down
			history[cur.height] = cur
			cur = cur.down 
		elseif self.le(cur.next.value, v) then 
			-- move right
			cur = cur.next
		end
	end
...
local function delete(self, v)
	local cur = self._head 
	local eq = self.eq or function(a, b) return self.le(a, b) and self.le(b, a) end
	local ge = function(a, b) return self.le(b, a) end
	while cur do 
		-- in a column before v and cut ties with it
		if cur.next and eq(cur.next.value, v) then 
			cur.next = cur.next.next 
			cur = cur.down
		elseif not cur.next or ge(cur.next.value, v) and cur.down then 
			-- move down
			cur = cur.down
		else
			-- move right
